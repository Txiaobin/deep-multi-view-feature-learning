...
#
# construction the time domain deep feature extraction network
# Calculate the time deep feature
...
def extracting_feature(mode, data, labels, steps, i, k):
	classifier = tf.estimator.Estimator(model_fn=cnn_model, params={
										...
										model_dir="model/loop_" + str(k) + "/data_" + str(i) + "/view1_model")
...
def cnn_model(features, labels, mode, params):
	...
	X_conv1 = tf.layers.conv2d(inputs=X, filters=1, kernel_size=[1, 128], padding='valid', activation=tf.nn.tanh)
	X_conv2 = tf.layers.conv2d(inputs=X_conv1, filters=30, kernel_size=[1, 65], padding='valid', activation=tf.nn.tanh)
	X_conv3 = tf.layers.conv2d(inputs=X_conv2, filters=20, kernel_size=[4, 33], padding='valid', activation=tf.nn.tanh)
	X_conv4 = tf.layers.conv2d(inputs=X_conv3, filters=10, kernel_size=[8, 18], padding='valid', activation=tf.nn.tanh)
	X_dense1 = tf.layers.dense(inputs=tf.reshape(X_conv4, [-1, 13*16*10]), units=1024, activation=tf.nn.tanh)
